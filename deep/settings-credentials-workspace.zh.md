...
> 读写与崩溃恢复已由[持久化深读](./persistence-crash-recovery.zh.md)承接，此处不重复。
...
[持久化深读](./persistence-crash-recovery.zh.md)）：消费方永不直触后端。
...
- 概览与浅层叙事：[配置面](../augmentation/settings-and-credentials.zh.md)
- 存储枢纽与后端、会话日志崩溃恢复：[持久化格式与崩溃恢复](./persistence-crash-recovery.zh.md)
- 注册即 fiber effect 的热重载语义：[Cordis 内核](../cordis/cordis-kernel.zh.md) ·
  [插件组装与启动](../cordis/plugin-composition.zh.md)
- 设置界面对 describe/凭据视图的消费：[host/client 分层与 API 网关](../platform/host-client-boundary.zh.md)
- workspace 作为 webhook/会话归属容器：[后台任务与外部触发](../augmentation/background-and-triggers.zh.md)
