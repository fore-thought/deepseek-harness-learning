...
## 相关
- 内核语义（epoch/effect/事件的地基）：[Cordis 内核](../cordis/cordis-kernel.zh.md)
- 配置树与启动序的概览：[插件组装与启动](../cordis/plugin-composition.zh.md)
- 启动链的应用壳半边：[应用壳](../platform/web-cli-boot.zh.md)
- 热重启“产品状态零扰动”的持久化侧印证：[持久化与崩溃恢复](./persistence-crash-recovery.zh.md)
