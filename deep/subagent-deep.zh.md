...
- 概览篇（本篇是其深读展开）：[委派与编排](../augmentation/subagent-orchestration.zh.md)
- AgentHandle 所有权与投递/取消：[Agent 主干深读](./agent-loop-internals.zh.md)
- fork 种子的日志契约：[会话事件日志](../agent-runtime/session-event-log.zh.md)、
  [持久化格式与崩溃恢复](./persistence-crash-recovery.zh.md)
- 身份/计时投影与缓存阶梯：[投影、遥测与格式迁移](./session-projection-telemetry.zh.md)
- 工作流如何复用本 seam 起子：[工作流与 agent team](./workflow-agent-team.zh.md)
- ACP 网关面（host↔client）：[host/client 分层与 API 网关](../platform/host-client-boundary.zh.md)
