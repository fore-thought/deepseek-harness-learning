...
> Host 侧网关与代码生成边界见 [host/client 分层与 API 网关](../platform/host-client-boundary.md)，
> 本篇只写浏览器这一侧，不重复。
...
- **vendored Loader 的唯一消费点**是 `EntryTree.import`——模块系统是"插件代码如何到达
  浏览器"的唯一可替换实现（[Cordis 热重启](./cordis-hot-reload.md)的浏览器镜像）。
...
这与 [会话事件日志](../agent-runtime/session-event-log.md)的"唯一真源+投影"哲学同构——
浏览器只是又一个投影消费者。
...
Assistant 状态**（与 [表层改写与压缩](./surface-compaction.md)的"模型侧表面"互为一体两面：
...
- **`ui-conversation`（复杂度上限 exemplar）**：乐观提交流水——Enter 同事务清草稿/
  ...
  见 [人机问答与反馈](../augmentation/questions-and-answers.md)）。
...
- Host 侧边界与 typert 生成：[host/client 分层与 API 网关](../platform/host-client-boundary.md)
  · [typert 协议深读](./typert-remote-protocol.md)
- 事件日志与投影：[会话事件日志](../agent-runtime/session-event-log.md)
- 主干装配（对照"第二个应用"的第一应用）：[插件组装与启动](../cordis/plugin-composition.md)
- 工具呈现的运行时侧：[工具注册表与执行流水线](../agent-runtime/tools-pipeline.md)
