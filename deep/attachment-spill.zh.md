...
> [上下文工程](../llm-layer/context-engineering.zh.md) 附件/溢出两节的源码级展开。
...
- 与 [会话持久化](../platform/session-persistence.zh.md) 恰成镜像：append 流是
...
- 执行序不变量：全部静态门在任何 fs I/O 之前（拒绝不泄漏半读/孤儿写）；无扩展名路径
  ...
  ——附件仓是宿主侧服务 I/O，这条与 [沙箱执行](./sandbox-execution.zh.md) 的边界值得说破。
...
- 第二 arm `tools/ptc-dispatch-log`：把 PTC 子调用结果的**日志副本**同样压成
  ...
  替换函数，产出字节一致的投影（见 [PTC 与 code-runtime](./ptc-code-runtime.zh.md)）。
...
- 概览与计量：[上下文工程](../llm-layer/context-engineering.zh.md)
- 引用块所在的事件语义：[会话事件日志](../agent-runtime/session-event-log.zh.md)
- post-execute 瀑布：[工具注册表与执行流水线](../agent-runtime/tools-pipeline.zh.md)
- 文件块"句柄文本投影"与表层改写：[表层改写与压缩](./surface-compaction.zh.md)
