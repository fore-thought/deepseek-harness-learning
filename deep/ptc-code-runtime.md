...
> [代码运行、LSP 与远程世界](../execution/remote-and-code-runtime.md) 与[工具注册表与执行
> 流水线](../agent-runtime/tools-pipeline.md)。证据基准：DSH 仓库源码直读 + 本机真机实测。
...
- **入站敌意闸 = 形状重建 + 幂等，不防说谎**（parseWorkerMessage，L133-165）：对端跑模型代码，
  ...
  **防事故不防作恶**，真实安全边界在审批/沙箱 seam（见 [沙箱执行](./sandbox-execution.md)）。
...
- [代码运行、LSP 与远程世界](../execution/remote-and-code-runtime.md) —— 本篇的概览篇
- [工具注册表与执行流水线](../agent-runtime/tools-pipeline.md) —— 被复刻的 native 调度器与坍缩挂点
- [会话事件日志：唯一真源](../agent-runtime/session-event-log.md) —— log-only 派发事件的持久化去处
- [沙箱执行](./sandbox-execution.md) —— run_code 代码真正受制的另一半边界
- [turn/step 主循环](../agent-runtime/turn-step-loop.md) —— commit 与 deferContext 的消费侧
