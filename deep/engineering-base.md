...
[附件与溢出篇](./attachment-spill.md) 写过 spill"预览机制另属 util/output-retention"，
...
Linux 沙箱消费内幕见 [沙箱执行](./sandbox-execution.md)（"~300 行 C11+musl 静态链接"
...
2. **llm-replay**：把**录制的会话日志本身**当测试脚本——从 v2 嵌入的 assistant 流与
   ...
   **格式迁移与回放脚本的锁步**是这套体系的地基（持久化勘误背景见
   [投影/遥测/代次迁移](./session-projection-telemetry.md)，在途篇按名引用）。
...
- 原子写镜像与 fsync 分层：[持久化格式与崩溃恢复](./persistence-crash-recovery.md)
- 预览代数的下游大用户：[附件与溢出](./attachment-spill.md)
- landlock 消费侧（拒绝分类/令牌）：[沙箱执行](./sandbox-execution.md)
- 品牌化模式的领域应用：[主干内幕](./agent-loop-internals.md)
- 配置与凭据面（atomic-write 消费方）：「配置·凭据·工作区深读」篇（N14 在途，
  成文后由主会话回填为相对链接 `deep/settings-credentials-workspace.md`）
- 应用壳的 DSH_* 供给上游：[应用壳](../platform/web-cli-boot.md)
