...
> 会话事件日志是唯一真源（[会话事件日志](../agent-runtime/session-event-log.md)）；本篇讲**怎么把它读回来**：
...
> 归 [投影与遥测内幕](./session-projection-telemetry.md)，本篇只写到查询侧怎么消费它们。
...
- 冷读底座 `readColdSessionLog`（cold-read.ts）：read handle 读完整校验日志 +
  ...
  [持久化深读](./persistence-crash-recovery.md)「冷读者」一节同文互证。
...
- 真源与折叠状态机：[会话事件日志](../agent-runtime/session-event-log.md)
- revision 令牌、flush 屏障、撕裂尾配平：[持久化格式与崩溃恢复](./persistence-crash-recovery.md)
- 表层改写（shadowed 的成因方）：[表层改写与压缩](./surface-compaction.md)
- 投影/标题/遥测供给侧：[投影与遥测内幕](./session-projection-telemetry.md)
- 会话引用注入线：[提示词组装与运行时上下文](./prompt-assembly-context.md)
- 委派树的谱系来源：[委派与编排](../augmentation/subagent-orchestration.md)
- 持久化概览（接缝归属层）：[会话持久化与存储](../platform/session-persistence.md)
