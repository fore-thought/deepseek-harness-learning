...
刷新机制的详述见[提示词组装与运行时上下文](./prompt-assembly-context.zh.md)）。
...
- 概览篇的压缩与计量分工：[上下文工程：计量、压缩、附件与溢出](../llm-layer/context-engineering.zh.md)
- 表层寄主的事件日志与投影框架：[会话事件日志：唯一真源](../agent-runtime/session-event-log.zh.md)
- 压缩挂点的轮次机器：[turn/step 主循环：agent loop 的心脏](../agent-runtime/turn-step-loop.zh.md)
- jsonl 写路径与 flush 屏障：[会话持久化与存储](../platform/session-persistence.zh.md)
- 括号锁 / end-seed 豁免的崩溃面姊妹篇：[持久化与崩溃恢复](./persistence-crash-recovery.zh.md)
- `startsRequestSeries` 的唯一生产者：
  [自组织：goal、plan mode、todo、schedule](../augmentation/goal-plan-todo.zh.md)
- token-meter 两轨表面与 shadow-price 协议展开：[LLM 适配器与计量内幕](./llm-adapters-metering.zh.md)
- 投影/缓存/格式代次的派生侧姊妹篇：[派生侧深读](./session-projection-telemetry.zh.md)
