...
| 概览篇说法（[代码运行、LSP 与远程世界](../execution/remote-and-code-runtime.zh.md)） | 深读结论 |
|---|---|
...
- **editText** 与本地同名同语义：唯一匹配（多义 `FS_AMBIGUOUS_EDIT`）、
  ...
  [沙箱执行](./sandbox-execution.zh.md) 的本地结构化拒绝分类，这是远程的
  降级面之一。
...
Consumer）作为部署形态开关的含义：[沙箱执行](./sandbox-execution.zh.md) 把本地
...
- 概览：[代码运行、LSP 与远程世界](../execution/remote-and-code-runtime.zh.md) ·
  [文件系统与沙箱](../execution/filesystem-and-sandbox.zh.md)
- 设计决定语境：[总览](../overview.zh.md)「能力 seam 三件套」
- 本地执行对照：[沙箱执行内幕](./sandbox-execution.zh.md)
- run_code/worker-thread 对照：[PTC 与 code-runtime 内幕](./ptc-code-runtime.zh.md)
