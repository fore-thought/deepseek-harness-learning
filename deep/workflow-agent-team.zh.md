...
- **四枚领域事件**（types.ts 声明合并进 `SessionEventMap`）：`team/member`、`team/task`、
  ...
  [会话投影与遥测](./session-projection-telemetry.zh.md)。）
- **事务与读侧**：`TeamJournal`（73 行）按 Lead Session 串行化 mutation 尾链，`appendAndFlush`
  ...
  unknown 标签）。这是[会话事件日志](../agent-runtime/session-event-log.zh.md)"投影派生"主线在
  协作域的满血消费者。
...
[插件组装](../cordis/plugin-composition.zh.md) 的 profile 叠加。
...
- 概览承接：[委派与编排](../augmentation/subagent-orchestration.zh.md)（本篇深读其 workflow/Ralph/team 三节）
- 孩子从哪来：[会话事件日志](../agent-runtime/session-event-log.zh.md)；持久底座：
  [会话持久化与存储](../platform/session-persistence.zh.md)
- 代次与投影机制：[会话投影与遥测](./session-projection-telemetry.zh.md)（同阶段 R1 篇）
- 工具执行与结构化输出子集：[工具注册表与执行流水线](../agent-runtime/tools-pipeline.zh.md)
- code seam 家族：[PTC 与 code-runtime](./ptc-code-runtime.zh.md)；profile/patch 形态：
