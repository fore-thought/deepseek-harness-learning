...
- **web**（88 行，三段式）：①surface 专属值（persona 重述、sqlite 全文检索
  ...
  `window.__DSH_BOOT__`，见 [应用壳](../platform/web-cli-boot.zh.md)）。
...
见 [host/client 边界](../platform/host-client-boundary.zh.md)）；publish 只发 `dist`，映射表
...
- 装载机制与 patch 语义：[插件组装与启动](../cordis/plugin-composition.zh.md)
- 重载的模块级内幕：[Cordis 热重启与热重载](./cordis-hot-reload.zh.md)
- 浏览器侧 boot 全链：[应用壳](../platform/web-cli-boot.zh.md)
- 双 face 传输与信任栅栏：[host/client 分层与 API 网关](../platform/host-client-boundary.zh.md)
- 一次性执行的 worker 后端：[PTC 与 code-runtime](./ptc-code-runtime.zh.md)
