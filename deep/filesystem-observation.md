...
> [概览篇](../execution/filesystem-and-sandbox.md)画出了 fs seam 与沙箱的地图，
> [沙箱执行内幕](./sandbox-execution.md)走读了内核边界那半边。本篇补齐剩下的两大块：
...
- containment 判定 `isPathUnder`：词法快路径（大小写敏感性按平台）→ 不等则
  **祖先逐跳 dev/ino 身份比对**，Windows 8.3 短名与大小写别名骗不过去；
  真机复放见 [沙箱执行内幕](./sandbox-execution.md)。
...
- 远程后端 fs-e2b 不在本篇——见概览篇提供方配对表与 [远程世界](../execution/remote-and-code-runtime.md)。
...
- seam 地图与提权审批流：[文件系统与沙箱](../execution/filesystem-and-sandbox.md)
- 内核边界与 denial 分类：[沙箱执行内幕](./sandbox-execution.md)
- 工具注册表（本组工具的宿主）：[工具流水线](../agent-runtime/tools-pipeline.md)
- 图片附件与溢出词汇：[上下文工程](../llm-layer/context-engineering.md)
- 远程后端与 LSP：[代码运行、LSP 与远程世界](../execution/remote-and-code-runtime.md)
