...
> [应用壳](../platform/web-cli-boot.md)「SDK：进程外正门」一句，本篇展开。
...
[会话事件日志](../agent-runtime/session-event-log.md)）。其二，locality 判定
...
（见 [委派编排深读](./subagent-deep.md)）：`inheritsParentContext = false`
...
- 四孔总览与 CLI 生命周期：[应用壳](../platform/web-cli-boot.md)
- 事件词汇同源处：[会话事件日志](../agent-runtime/session-event-log.md)
- 图片 admission 契约：[附件与溢出](./attachment-spill.md)
- 进程内委派对照面：[子代理深读](./subagent-deep.md)
- seam"整组搬家"与本文例外的语境：[LSP 与 E2B 远程世界](./lsp-e2b-remote.md)
