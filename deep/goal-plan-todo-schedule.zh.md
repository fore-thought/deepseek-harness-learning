...
- 概览与浅层叙事：[自组织](../augmentation/goal-plan-todo.zh.md)
- `runMaintenance`/inbox/pre-step 的执行侧：[agent-loop 内幕](./agent-loop-internals.zh.md)（同批产出）
- 事件日志与投影框架：[会话事件日志](../agent-runtime/session-event-log.zh.md)
  · [会话持久化与存储](../platform/session-persistence.zh.md)
- 评审通道复用：[人机问答与反馈](../augmentation/questions-and-answers.zh.md)
- `@Remote` 远程面：[host/client 分层与 API 网关](../platform/host-client-boundary.zh.md)
