...
> 全库头号主线缺口的承接篇：[总览](../overview.md)导航表把"提示词组装"写在 Agent 主干行里，
...
| [会话事件日志](../agent-runtime/session-event-log.md)：`request/header` 携带"渲染后 system+tools+配置" | 补精确：只有这三样进纪元头；AGENTS.md 基线、上下文快照、时间/tmux 注入都以 `user/message` **另行入史**——提示词 ≠ 历史 |
| [turn/step 主循环](../agent-runtime/turn-step-loop.md)的 `agent/pre-step` | 补消费者谱系：循环本体（快照）、上下文插件（prepend 注入）、agent-instructions（后序折入）三层 |
| [surface-compaction](./surface-compaction.md)："`<system-reminder>` 由 agent-instructions 烤进 content" | 该框架、预算与触敏刷新机制在本篇展开 |
...
[持久化深读](./persistence-crash-recovery.md)实测"125 步仅 1 条纪元头、35KB 单行"——
...
它与表层/压缩的交互在 [surface-compaction](./surface-compaction.md) 已展开，本篇不重复。
...
（见 [host/client 边界](../platform/host-client-boundary.md)）。
...
`composeFrom(parent)` 认父继承其组装（见[委派与编排](../augmentation/subagent-orchestration.md)）。
...
- 缺口承接对象：[总览](../overview.md) · [会话事件日志](../agent-runtime/session-event-log.md)
- 消费侧：[turn/step 主循环](../agent-runtime/turn-step-loop.md) ·
  [工具注册表与执行流水线](../agent-runtime/tools-pipeline.md)（schema 供给方）
- 相邻层：[上下文工程](../llm-layer/context-engineering.md)（attachment/compaction/spill）
- 深读姐妹篇：[表层改写与压缩](./surface-compaction.md)（快照与基线的表层命运） ·
  [持久化格式与崩溃恢复](./persistence-crash-recovery.md)（纪元头落盘实测）
- 委派与挂载：[委派与编排](../augmentation/subagent-orchestration.md) ·
  [能力供给](../augmentation/skills-mcp-hooks.md) ·
  [插件组装与启动](../cordis/plugin-composition.md)（作用域遮蔽为何成立）
