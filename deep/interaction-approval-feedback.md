...
> 概览篇 [人机问答与反馈](../augmentation/questions-and-answers.md) 的深读承接：
...
| messageFeedback "两域之一" | 与持久化篇统一口径：全库 storage-domain **3 域**
  ...
  [持久化深读](./persistence-crash-recovery.md)。
...
- 远程面：服务继承 `TypertRemoteService`，`list`/`execute` 标 `@Remote`——浏览器端
  命令面板经 [typert 协议](./typert-remote-protocol.md) 直调。
...
- 全 Remote：list/put/delete 标 `@Remote('list'|'put'|'delete')`，浏览器直调
  （[host/client 边界](../platform/host-client-boundary.md)）。
...
- 概览篇：[人机问答与反馈](../augmentation/questions-and-answers.md)
- 审批的调用方：[工具注册表与执行流水线](../agent-runtime/tools-pipeline.md)
- 旋钮执行侧：[沙箱执行内幕](./sandbox-execution.md) ·
  [文件系统与沙箱](../execution/filesystem-and-sandbox.md)
- 日志语义暗线的另一端：[持久化格式与崩溃恢复](./persistence-crash-recovery.md)、
  投影机制 [会话投影与遥测](./session-projection-telemetry.md)
- 跨进程传输：[typert 远程协议](./typert-remote-protocol.md)
