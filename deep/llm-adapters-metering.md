...
> 本篇是 [LLM 层概览](../llm-layer/llm-vocabulary.md) 的源码级深读展开：`ctx.llm` 注册表
...
重写纪元头是常态（[持久化格式与崩溃恢复](./persistence-crash-recovery.md) 的 6049 帧
...
- 范围说明：模型选择 UI（client 组）与 settings/credentials 接缝本体不在本篇——见
  [host/client 分层](../platform/host-client-boundary.md) 与
  [配置面](../augmentation/settings-and-credentials.md)。

## 相关

- 概览篇（本篇是其深读展开）：[LLM 层](../llm-layer/llm-vocabulary.md)
- 谁驱动 stream 与 header：[turn/step 主循环](../agent-runtime/turn-step-loop.md)、
  [会话事件日志](../agent-runtime/session-event-log.md)
- system/tools 槽位的来源：[提示词组装与上下文注入](./prompt-assembly-context.md)
  （同期产出）
- 压缩消费计量数字：[表层改写与压缩](./surface-compaction.md)、
  [上下文工程](../llm-layer/context-engineering.md)
- 紧凑流行的物理侧与 usage 遥测：[持久化格式与崩溃恢复](./persistence-crash-recovery.md)
- 凭据/设置的接缝语义：[配置面](../augmentation/settings-and-credentials.md)
- 投影单元的注册与缓存：[投影、遥测与格式迁移](./session-projection-telemetry.md)
  （同期产出）
