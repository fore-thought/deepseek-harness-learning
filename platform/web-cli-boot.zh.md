...
（入口纪律由 launcher 校验，见 [插件组装与启动](../cordis/plugin-composition.zh.md)；
"孔"指应用面而非代码分支——六 bundle 装配差异见 [Profile 组装与启动](../deep/boot-bundles.zh.md)）。
...
[SDK 三件套深读](../deep/sdk-embedding.zh.md)。
...
（把轮次委派给另一个 ACP 产品，见 [委派与编排](../augmentation/subagent-orchestration.zh.md)；
`packages/acp`=对外 server 与 `subagent-acp`=对外驱动是**两包两面孔**，见
[委派内幕](../deep/subagent-deep.zh.md)）。

## headless：一次性运行器

`dsh-headless`：不带服务器跑完任务即退（`config.task:
!!js ctx.headlessStartup.task`——命令行 flag 值经服务注入门控流入插件配置，
[插件组装与启动](../cordis/plugin-composition.zh.md) 的表达式机制用例）。CI 与脚本集成的孔。

## 相关

- 启动装配全链：[插件组装与启动](../cordis/plugin-composition.zh.md)
- 浏览器对话的传输线：[host/client 分层与 API 网关](./host-client-boundary.zh.md)
