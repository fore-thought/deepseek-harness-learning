...
> [持久化格式与崩溃恢复](./persistence-crash-recovery.md)讲的是日志的**写侧**（帧、写门、
...
- 写侧姊妹篇（帧格式在彼处，本篇不重复）：[持久化格式与崩溃恢复](./persistence-crash-recovery.md)
- 概览篇（投影/租约说法的出处）：[会话持久化与存储](../platform/session-persistence.md)
- 事件本体（v2 词表修正的落点）：[会话事件日志](../agent-runtime/session-event-log.md)
- 投影消费者（历史尾页/推送帧）：[host/client 分层与 API 网关](../platform/host-client-boundary.md)
- 反馈事件（FEEDBACK_ONLY 的触发源）：[人机问答与反馈](../augmentation/questions-and-answers.md)
- 配置面（patch 层写触发/模式值的读法）：[配置面](../augmentation/settings-and-credentials.md)
