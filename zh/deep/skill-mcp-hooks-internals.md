...
> [概览篇](../augmentation/skills-mcp-hooks.md) 的 skills/MCP/hooks 三节深读展开。
> commands 归交互组承接篇、preset 归[提示词组装](./prompt-assembly-context.md)、
...
注册表与[工具注册表](../agent-runtime/tools-pipeline.md)同形（host+per-scope 层）：
...
会话技能目录走[事件日志](../agent-runtime/session-event-log.md)路线而非
...
- **图片结果=信任边界全景**：mediaType 限四格式、canonical base64 解码重编码
  ...
  模型上下文与 PTC 值各保各的形状（详见[附件与溢出](./attachment-spill.md)）。
...
- 边界两条：`taskSupport:'required'` 的工具直接运行期拒绝（桥不支持任务型执行）；
  ...
  [子进程](../execution/shell-process-terminal.md)世界的待遇不同。
...
- 概览：[能力供给](../augmentation/skills-mcp-hooks.md)；命令/审批：
  [工具流水线](../agent-runtime/tools-pipeline.md)
- 图片入上下文：[附件与溢出](./attachment-spill.md)；PTC 值形状：
  [PTC 与 code-runtime](./ptc-code-runtime.md)
- 注入落位与表层：[会话事件日志](../agent-runtime/session-event-log.md) ·
  [表层改写与压缩](./surface-compaction.md)
- preset 深读：[提示词组装与上下文注入](./prompt-assembly-context.md)
- extensions 动态化：[Cordis 热重启](./cordis-hot-reload.md)
...
- 目录"追加+框文声明替换"与表层 replace 的交互（旧目录是否会被压缩改写）
  未对照 [表层改写与压缩](./surface-compaction.md) 实测时间线。
