...
> 本篇是 [上下文工程](../llm-layer/context-engineering.md) 与
> [工具流水线](../agent-runtime/tools-pipeline.md) 都未展开的一块补全：
...
guard 组 `timeout-policy`，见 [进程与终端深读](./shell-terminal-internals.md)）；
`fetchMaxOutputChars = 200000`。两工具各注册一段 system-prompt 引导
（`tool:web_search`/`tool:web_fetch`，协作式组装——提示词组装机制另见
[prompt 组装篇](./prompt-assembly-context.md)）：**本会话（fork 运行时）工具描述
...
- **辅助模型调用的日志义务**：`recordRequest` 在派发**前**把去密钥化的精确请求
  ...
  见 [会话事件日志](../agent-runtime/session-event-log.md)）。
...
（格式代次命名，详见 [投影与迁移篇](./session-projection-telemetry.md)）。

## 相关

- 提示词段落协作式组装：[系统提示与上下文注入](./prompt-assembly-context.md)
- 工具注册/并发安全/展示 meta 契约：[工具流水线](../agent-runtime/tools-pipeline.md)
- 超时预算 enforcement 与 guard 组：[进程与终端内幕](./shell-terminal-internals.md)
- 附件与溢出的姊妹截断策略（字节/字符帽对照）：[附件与溢出](./attachment-spill.md)
