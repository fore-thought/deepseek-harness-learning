...
> [后台任务与外部触发](../augmentation/background-and-triggers.md) 两节的源码级展开。
...
| 概览篇说法 | 深读结论 |
|---|---|
| 生产方"bash 后台、subagent 后台、workflow……" | **workflow 不走 jobs**：全库 `jobs.start(` 实测仅 4 处调用点——tool-bash / tool-pwsh / tool-subagent / tool-terminal（`pty-send`）[MEASURED]；workflow 的可观察性是 `workflow/*` 事件对（见 [工作流与 Agent Teams](./workflow-agent-team.md)），不进作业注册表 |
...
- 概览浅层：[后台任务与外部触发](../augmentation/background-and-triggers.md)
- 三家 producer 的执行体内幕：[Shell 与终端内幕](./shell-terminal-internals.md) ·
  [委派内幕](./subagent-deep.md)
- 不走 jobs 的编排通道：[工作流与 Agent Teams](./workflow-agent-team.md)
- followup/inbox 的合流闸：[turn/step 主循环](../agent-runtime/turn-step-loop.md)
- 品牌化 id 与 ScopedLayers 底座：[工程基建](./engineering-base.md) ·
  [主干循环内幕](./agent-loop-internals.md)
...
- `pty-send` 与 terminal 会话生命周期的耦合细节归
  [Shell 与终端内幕](./shell-terminal-internals.md) 口径，本篇只在 producer 映射里点名。
