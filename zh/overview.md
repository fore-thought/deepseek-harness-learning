...
| Agent 主干 | 会话日志、turn/step 循环、工具流水线、提示词组装 | [会话事件日志](./agent-runtime/session-event-log.md) · [turn/step 主循环](./agent-runtime/turn-step-loop.md) · [工具注册表与执行流水线](./agent-runtime/tools-pipeline.md) · 深读：[主干循环内幕](./deep/agent-loop-internals.md) · [提示词组装与上下文](./deep/prompt-assembly-context.md) |
...
| 平台层 | 持久化、storage、host/client、API 网关、应用壳 | [会话持久化与存储](./platform/session-persistence.md) · [host/client 分层与 API 网关](./platform/host-client-boundary.md) · [应用壳](./platform/web-cli-boot.md) · 深读：[派生侧内幕](./deep/session-projection-telemetry.md) · [会话查询内幕](./deep/session-query.md) |
...
| 专题深读 | 29 篇源码级走读与真机实测（`deep/`，组级全覆盖 51 包组，含推翻性修正） | [承接表与深读索引](./dsh-harness-index.md) |
...
2. **模型可见即已记录**：抵达模型的一切必须能从事件日志重建，并有运行时不变量断言
   ——断言的执法层是[不变量注册表](./deep/invariants-registry.md)；**挂载面有两口径**：
...
3. **能力 seam 三件套**：Service Definition + Provider + Consumer 一起设计；
   ...
   活标本与验证：[LSP 与 E2B 远程世界](./deep/lsp-e2b-remote.md)（逐环接线链）、
   [Web 工具](./deep/web-tools.md)（一 seam 两操作四提供方的最干净形状）；
   成文的例外=SDK client（裸 spawn 共享环境清洗基座，见 [SDK 三件套深读](./deep/sdk-embedding.md)）。
...
- monorepo：`packages/` 51 组、`apps/{cli,web}`、`vendor/`（cordis 等 8 个上游同步副本）、
  ...
  （机制组织见 [工程基建深读](./deep/engineering-base.md)）。
