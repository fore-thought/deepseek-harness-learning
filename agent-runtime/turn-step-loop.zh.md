...
- **取消**：`cancel(cause, {keepInbox})` → 默认清 inbox（durable canceled；`keepInbox` 则
  ...
  **先分类再插入**。崩溃时未闭合轮由 resume 合成 interrupted（见 [会话事件日志](./session-event-log.zh.md)）。
- **创建事务**：`prepare()` 三源融合取消（调用方 signal/所属 fiber 卸载/factory 拆迁）；
  ...
  （[工具注册表与执行流水线](./tools-pipeline.zh.md) 的作用域层由它派生）。
...
`request/context` 三元组差量见 [主干循环内幕](../deep/agent-loop-internals.zh.md) 与
...
- 事件词汇与投影：[会话事件日志](./session-event-log.zh.md)
- 步骤内工具怎么跑：[工具注册表与执行流水线](./tools-pipeline.zh.md)
- 循环挂上来的增强策略：[自组织](../augmentation/goal-plan-todo.zh.md) ·
  [上下文工程](../llm-layer/context-engineering.zh.md)
- 主干实现层内幕：[agent-loop-internals](../deep/agent-loop-internals.zh.md)；pre-step 消费者谱系：
