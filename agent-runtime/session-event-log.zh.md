...
| `assistant/chunk` | 原始流式分块（UI 保真与回放的原料；**代次敏感**——v2 持久词表已无顶层 chunk 行，released dispositions 删籍，见[派生侧深读](../deep/session-projection-telemetry.zh.md)） |
...
- `SurfaceOp = 'append' | { op:'replace', start, end }`：**压缩改写历史不是删日志，
  ...
  [表层改写与压缩](../deep/surface-compaction.zh.md)。
- 铁律"**模型可见即已记录**"：任何抵达模型请求的输入都必须能从日志重建，
  由运行时不变量断言（机制速写见 [插件组装与启动](../cordis/plugin-composition.zh.md)；
  注册表与 39 伴生的执法全貌见 [不变量注册表](../deep/invariants-registry.zh.md)）。
...
- 写失败**尽力而为**、不阻断对话（日志可缺尾）；`flush` 才是持久性屏障（见 [会话持久化与存储](../platform/session-persistence.zh.md)）。
- resume 时 `interruptedTurnClosers`：对未闭合轮在**写句柄内**合成补齐
  ...
  `agent-loop.resumeWith` 经同一写句柄 append；与[持久化篇](../deep/persistence-crash-recovery.zh.md)
...
- 谁在写这些事件：[turn/step 主循环](./turn-step-loop.zh.md)
  · [会话持久化与存储](../platform/session-persistence.zh.md)（怎么落盘）
- 投影怎么送到浏览器：[host/client 分层与 API 网关](../platform/host-client-boundary.zh.md)
- 表层折叠/压缩事务全细节：[表层改写与压缩](../deep/surface-compaction.zh.md)
- 落盘格式与撕裂尾恢复实测定量：[持久化格式与崩溃恢复](../deep/persistence-crash-recovery.zh.md)
- 纪元头比较语义（initial/resume/change/series）与双通道落点：
  [提示词组装与上下文](../deep/prompt-assembly-context.zh.md)
- 投影/标题/遥测/格式代次的实现层全解：[派生侧深读](../deep/session-projection-telemetry.zh.md)
- 查询侧的 surface 三态与派生：与模型历史推导共用同一 `foldSurface`、不发明标签——
  [会话查询内幕](../deep/session-query.zh.md)
