...
- 委派对象的创建/取消语义：[turn/step 主循环](../agent-runtime/turn-step-loop.zh.md)
- fork 种子的日志边界：[会话事件日志](../agent-runtime/session-event-log.zh.md)
- ACP 双面孔（被委派目标/对外服务器）：[应用壳](../platform/web-cli-boot.zh.md)
- 实现层内幕（能力双闸门/Activation 驻留/四路传输/结算竞态窗口）：
  [委派内幕](../deep/subagent-deep.zh.md)
- 工作流引擎与 Agent Teams 深读（上限表/配对账本/mailbox 语义）：
  [工作流与 Agent Teams](../deep/workflow-agent-team.zh.md)
- 进程外嵌入通道（"不过对话边界"的后端）：[SDK 三件套深读](../deep/sdk-embedding.zh.md)
