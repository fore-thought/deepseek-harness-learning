...
- `JobRegistry` 抽象 seam + `LocalJobRegistry` provider；生产方是工具
  （bash 后台、subagent 后台、workflow……）声明 `JobStart{kind, label, hooks}`；
  `JobKindMap` 声明合并扩展 kind 词汇（bash/subagent/…）。
...
- 生命周期钩子 `JobHooks{cancel, done, readOutput}`——注册表不关心任务本体语义，
  **bash 进程与子 agent 会话在同一生命周期协议下并存**（你轮询我六路子代理用的
  就是这同一条 `job_output` 线）；
