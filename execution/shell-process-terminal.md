...
- 前台 bash 链：提权参数校验 → `ctx.sandboxPolicy.resolve` →
  ...
  `confine(['bash','-c',cmd], policy)` 包装（见 [文件系统与沙箱](./filesystem-and-sandbox.md)）；
- **shell 不管后台身份**：返回无任务概念的进程句柄，`job` 语义
  （id=`<kind>-N`、owner 鉴权、first-wins）归 `ctx.jobs` 适配层
  （[后台任务与外部触发](../augmentation/background-and-triggers.md)）——关注点切分干净。
- `runHook` 通道为外部 hooks 生态定制（清洗环境/进程组取消/超时，
  见 [能力供给](../augmentation/skills-mcp-hooks.md)）。
