...
- 消费方=面向模型的 `tool-lsp`；**没有让模型直接操纵 LSP 会话状态**——
  ...
  （对照 [能力供给](../augmentation/skills-mcp-hooks.zh.md) 的 MCP 只桥 Tools）；
  默认值全集（帧/文档/拆除预算）与取消的有界语义见
  [LSP 与 E2B 远程世界](../deep/lsp-e2b-remote.zh.md)。

## E2B：远程执行世界的完整标本（组 README 自述实验性 POC，无已发布组合默认启用）

`packages/e2b/`：`ctx.e2b` 共享单一远程 SDK 句柄；
**fs-e2b + subprocess-e2b 成对替换本地世界**（[文件系统与沙箱](./filesystem-and-sandbox.zh.md)）；
...
- 执行世界配对总论：[文件系统与沙箱](./filesystem-and-sandbox.zh.md)
- 工具流水线：[工具注册表与执行流水线](../agent-runtime/tools-pipeline.zh.md)
- 远程化后 UI 如何跟进：[host/client 分层与 API 网关](../platform/host-client-boundary.zh.md)
- run_code 桥/调度器/敌意闸与 21 项真机实测：[PTC 与 code-runtime 内幕](../deep/ptc-code-runtime.zh.md)
- `ctx.remote` 背后的协议与生成器：[typert 远程协议与代码生成](../deep/typert-remote-protocol.zh.md)
