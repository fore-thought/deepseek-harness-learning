...
2. 组合 patch 层（`structuredClone` 防 insert 引用别名污染；**空根 `cordis.yml`
   ...
   六 bundle 各装什么、差异矩阵见 [Profile 组装与启动](../deep/boot-bundles.zh.md)；
...
7. `appReady.commit()` → web profile 起 HTTP 监听并开浏览器（见 [应用壳](../platform/web-cli-boot.zh.md)）。
...
任何插件的注册都是 effect（[Cordis 内核](./cordis-kernel.zh.md)），所以"卸载→重载" = 逆序回卷 + 重放，
产品状态（会话日志）不受影响。三级决策表与双缓存失效细节见 [Cordis 热重启与热重载内幕](../deep/cordis-hot-reload.zh.md)。
**这是 JS 运行时私有技巧**：依赖 Node ESM 内部
loadCache/ModuleJob 结构才能成立。

## 相关

- 内核语义：[Cordis 内核](./cordis-kernel.zh.md)
- 启动后的应用壳与浏览器端 boot：[应用壳](../platform/web-cli-boot.zh.md)
- host/client 进程边界：[host/client 分层与 API 网关](../platform/host-client-boundary.zh.md)
