...
- 双轨估算：图片按**路由定价**（visualTokens+模型可见文本），文本用固定启发式
  ...
  与 shadow-price 协议展开见 [LLM 适配器与计量内幕](../deep/llm-adapters-metering.zh.md)；
...
5. append `compaction/summary` + 带 `surfaceOp:{op:'replace'}` 的
   `user/message` 检查点（**改写表层不动日志**，见 [会话事件日志](../agent-runtime/session-event-log.zh.md)）→
   `compaction/end` 释锁。
...
- 三级漏斗（admission→normalization→request variant）与落盘耐久全解：
  [附件与溢出](../deep/attachment-spill.zh.md)。
...
两武装（post-execute 三跳过 + ptc-dispatch-log）与预算代数见[附件与溢出](../deep/attachment-spill.zh.md)。
（你现在看到 grep 大结果"存盘并给路径"，就是这个机制。）

## 相关

- 计量喂给谁：[turn/step 主循环](../agent-runtime/turn-step-loop.zh.md) 的压力检查；
- provider 复用意图：[LLM 层](./llm-vocabulary.zh.md)；
- 表层 replace 语义：[会话事件日志](../agent-runtime/session-event-log.zh.md)
- 事务全序、常量组、影子价协议与真机验证：[表层改写与压缩](../deep/surface-compaction.zh.md)
- 分工澄清：本篇管**容量面**（计量/压缩/仓储），"抵达模型的内容如何合成"归
  [提示词组装与运行时上下文](../deep/prompt-assembly-context.zh.md)
